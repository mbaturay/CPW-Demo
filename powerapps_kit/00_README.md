...
## Known adjustments applied (post-generation)

- Fixed Status History datasource name in formulas to `cpw_StatusHistories` (was `cpw_StatusHistorys`). If your Power Apps datasource name differs, update it consistently.
- Fixed `cpw_ChangedBy` in StatusHistory patches to write the **user record** (`gblCurrentUser`) instead of a display-name string.
- Added required `cpw_HistoryLabel` to all StatusHistory Patch() calls so Dataverse inserts succeed.
- Added `09_control_naming_checklist.md` to help you rename controls before pasting formulas.
